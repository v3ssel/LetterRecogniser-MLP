...
LetterRecogniserWindow::LetterRecogniserWindow(QWidget* parent)
    ...
    s21::Controller::getInstance().makeMLP(
        ...
        ui_->learnrate_doubleSpinBox->value(), epoch_callback,
        process_callback);
...
void LetterRecogniserWindow::trainingButtonClicked() {
    if (ui_->start_training_pushButton->text().contains("Start")) {
        ...
        training_future_watcher_ =
            new QFutureWatcher<std::vector<double>>(this);
...
void LetterRecogniserWindow::testingResults() {
    ...
    try {
        ...
        ui_->recall_value_label->setText(
            QString::number(metrics.recall, 'g', 2));
        ui_->fm_value_label->setText(
            QString::number(metrics.f_measure, 'g', 2));
        ui_->time_value_label->setText(
            QString::number(metrics.testing_time.count() / 1000.0l, 'g', 2) +
            "s");
    } catch (const std::exception& ex) {
        QMessageBox::critical(this, "Failed to test model.",
                              QString("Error message: ") +
                                  QString::fromStdString(exception_msg_));
    }
...
void LetterRecogniserWindow::trainingResult() {
    ...
    try {
        ...
        QMessageBox::critical(this, "Failed to train model.",
                              QString("Error message: ") +
                                  QString::fromStdString(exception_msg_));
    }
...
s21::ModelType LetterRecogniserWindow::chooseModelType() {
    return ui_->model_comboBox->currentText() == "Matrix"
               ? s21::ModelType::Matrix
               : s21::ModelType::Graph;
}
