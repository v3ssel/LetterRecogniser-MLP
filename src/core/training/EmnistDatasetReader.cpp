...
namespace s21 {
    void EMNISTDatasetReader::open(const std::string &filename) {
        ...
        if (!_file.is_open()) {
            throw std::invalid_argument("EMNISTDatasetReader::open: Cannot open the file \"" + filename + "\".");
        }
    }

    EMNISTData EMNISTDatasetReader::readLine() {
        if (!_file.is_open()) {
            throw std::invalid_argument("EMNISTDatasetReader::readLine: File is not open.");
        }
        ...
        if (data.image.size() != 784) {
            throw std::runtime_error("EMNISTDatasetReader::readLine: EMNIST line must consist of 1 result and 784 numbers.");
        }
