...
std::unique_ptr<MLPModel> ModelBuilder::build() {
    if (model_type_ == ModelType::Matrix) {
        return std::make_unique<MatrixModel>(
            input_layer_size_, output_layer_size_, layers_, hidden_layer_size_,
            learning_rate_);
    }

    return std::make_unique<GraphModel>(input_layer_size_, output_layer_size_,
                                        layers_, hidden_layer_size_,
                                        learning_rate_);
}
}  // namespace s21