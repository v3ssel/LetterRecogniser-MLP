...
std::vector<double> MatrixModel::feedForward(
  ...
  for (size_t i = 0; i < _layers.size() - 1; ++i) {
    _layers[i + 1].values =
        ((_layers[i].values * _layers[i].weights) + _layers[i].bias);
...
void MatrixModel::randomFill() {
  for (size_t i = 0; i < _layers.size(); ++i) {
    _layers[i].weights = Matrix::GenerateRandom(_layers[i].weights.getRows(),
                                                _layers[i].weights.getCols());
    _layers[i].bias = Matrix::GenerateRandom(_layers[i].bias.getRows(),
                                             _layers[i].bias.getCols());
  }
}
...
std::vector<size_t> MatrixModel::getLayersSize() const {
  ...
  for (auto &layer : _layers) vec.push_back(layer.size);
...
void MatrixModel::setWeights(const std::vector<double> &weights) {
  size_t need_weights = std::accumulate(
      _layers.begin(), _layers.end(), 0, [](int i, MatrixLayer l) {
        return i + l.weights.getCols() * l.weights.getRows();
      });
  if (weights.size() != need_weights) {
    throw std::out_of_range(
        "Number of weights is not equal to the number of weights in the model");
  }
...
void MatrixModel::setBiases(const std::vector<double> &biases) {
  ...
  if (biases.size() != need_biases) {
    throw std::out_of_range(
        "Number of biases is not equal to the number of biases in the model");
  }
...
void MatrixModel::setLearningRate(double rate) { _learning_rate = rate; }

double MatrixModel::getLearningRate() const { return _learning_rate; }
}  // namespace s21
