...
namespace s21 {
void s21::FileMLPSerializer::serialize(std::unique_ptr<MLPModel> &model,
...
void s21::FileMLPSerializer::deserialize(std::unique_ptr<MLPModel> &model,
                                         const std::string &filename) {
...
std::vector<double> FileMLPSerializer::fillImportVector(std::ifstream &s,
...
std::pair<size_t, size_t> FileMLPSerializer::getImportSize(
    std::unique_ptr<MLPModel> &model, const std::string &line) {
  ...
  if (layers_info != model->getLayersSize()) {
    throw std::invalid_argument(
        "Filemodel does not correspond to the object model");
  }
