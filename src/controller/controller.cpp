...
namespace s21 {
    void Controller::makeMLP(ModelType type,
        ...
        std::unique_ptr<MLPSerializer> serializer = std::make_unique<FileMLPSerializer>();
        std::unique_ptr<MLPTrainer>trainer = std::make_unique<EMNISTMLPTrainer>(epoch_callback, process_callback);
