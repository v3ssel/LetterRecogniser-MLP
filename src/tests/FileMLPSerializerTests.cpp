...
const std::string kTestModelFilename =
...
TEST(FileMLPSerializer, SerializeMatrixModel) {
  ...
  std::unique_ptr<s21::MLPModel> rnd_model =
      std::make_unique<s21::MatrixModel>(10, 3, 2, 5, 0.4);
  rnd_model->randomFill();

  std::filesystem::path path(kTestModelFilename);
  std::string new_filename =
      path.replace_filename(path.stem().string() + "_serialized" +
  ...
  std::unique_ptr<s21::MLPModel> des_model =
      std::make_unique<s21::MatrixModel>(10, 3, 2, 5, 0.4);
...
TEST(FileMLPSerializer, DeserializeInvalidModel) {
  std::unique_ptr<s21::MLPSerializer> serializer =
      std::make_unique<s21::FileMLPSerializer>();
  std::unique_ptr<s21::MLPModel> model =
      std::make_unique<s21::MatrixModel>(784, 26, 2, 140, 0.1);

  EXPECT_THROW(serializer->deserialize(model, kTestModelFilename),
...
TEST(FileMLPSerializer, DeserializeInvalidFile) {
  ...
  EXPECT_THROW(serializer->deserialize(model, broken_filename),
               std::runtime_error);
}
