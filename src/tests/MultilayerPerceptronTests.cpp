...
const std::string kModelPath =
    (std::filesystem::current_path() / "tests" / "assets" / "model-test.txt")
        .string();
const std::string kSmartModelPath =
...
s21::MultilayerPerceptron getMLP() {
  std::unique_ptr<s21::MLPModel> model =
      std::make_unique<s21::MatrixModel>(10, 3, 2, 5, 0.3);
  model->randomFill();
...
TEST(MultilayerPerceptron, ImportExport) {
  ...
  std::string export_path =
      path.replace_filename(path.stem().string() + "-export" +
...
TEST(MultilayerPerceptron, TwoModels) {
  ...
  std::string m_model_path =
      path.replace_filename(path.stem().string() + "_matrix" +
                            path.extension().string())
          .string();
  std::string g_model_path =
      path.replace_filename(path.stem().string() + "_graph" +
                            path.extension().string())
          .string();
