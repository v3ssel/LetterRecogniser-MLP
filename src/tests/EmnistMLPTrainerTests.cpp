...
const std::string kDatasetPath =
    (std::filesystem::current_path() / "tests" / "assets" / "emnist-sample.txt")
        .string();
const std::string kModelPath =
    (std::filesystem::current_path() / "tests" / "assets" / "smartmodel.txt")
        .string();

TEST(EMNISTMLPTrainer, Training) {
  size_t ecb_count = 0, pcb_count = 0;
  std::vector<double> accurancy_percent;
  auto epoch_callback = [&ecb_count, &accurancy_percent](size_t, double,
                                                         double accurancy) {
    ecb_count++;
  ...
  std::unique_ptr<s21::MLPTrainer> trainer =
      std::make_unique<s21::EMNISTMLPTrainer>(epoch_callback, process_callback);
  std::unique_ptr<s21::MLPModel> model =
      std::make_unique<s21::MatrixModel>(784, 26, 5, 140, 0.1);
...
TEST(EMNISTMLPTrainer, Testing) {
  ...
  std::unique_ptr<s21::MLPTrainer> trainer =
      std::make_unique<s21::EMNISTMLPTrainer>([](size_t, double, double) {},
                                              process_callback);
  std::unique_ptr<s21::MLPModel> model =
      std::make_unique<s21::MatrixModel>(784, 26, 5, 140, 0.1);
  std::unique_ptr<s21::MLPSerializer> serializer =
...
TEST(EMNISTMLPTrainer, TrainWithBrokenDataset) {
  std::unique_ptr<s21::MLPTrainer> trainer =
      std::make_unique<s21::EMNISTMLPTrainer>(
          [](size_t, double, double) {}, [](size_t, s21::MLPTrainStages) {});
  std::unique_ptr<s21::MLPModel> model =
      std::make_unique<s21::MatrixModel>(784, 26, 5, 140, 0.1);

  std::filesystem::path path = kDatasetPath;
  std::string broken_dataset =
      path.replace_filename(path.stem().string() + "-broken" +
...
TEST(EMNISTMLPTrainer, CrossvalidationWithBrokenDataset) {
  ...
  EXPECT_THROW(trainer->crossValidation(model, broken_dataset, 1),
               std::runtime_error);
}
...
TEST(EMNISTMLPTrainer, CrossvalidationWithSoManyGroups) {
  ...
  EXPECT_THROW(trainer->crossValidation(model, broken_dataset, 1000),
               std::runtime_error);
}
...
TEST(EMNISTMLPTrainer, TestingWithHighPercent) {
  ...
  EXPECT_THROW(trainer->test(model, broken_dataset, 1000),
